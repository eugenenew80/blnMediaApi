...
@Stateless
public class AutoPowerConsumptionReader implements AutoReader<PowerConsumptionRaw> {
	private static final Logger logger = LoggerFactory.getLogger(AutoPowerConsumptionReader.class);

	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public void read() {
		logger.info("AutoPowerConsumptionReader.read started");
		...
		workListHeaderService.findAll().stream()
			...
			.forEach(header -> {
				...
				try {
					...
					logger.error("AutoPowerConsumptionReader.read failed: " + e.getMessage());
					errorBatch(header, batch, e);
				}
			});

		logger.info("AutoPowerConsumptionReader.read completed");
	}
